...
import LazyImage from '../../components/section-reusable-components/lazy-image';
...
const BlogCard = ({
  ...
  const renderSkeleton = () => {
    ...
    for (let index = 0; index < blog.limit; index++) {
      array.push(
        <div className="shadow-2xl card compact bg-base-100" key={index}>
  ...
  const renderArticles = () => {
    return articles && articles.length ? (
      articles.slice(0, blog.limit).map((article, index) => (
        <a
          className="shadow-2xl cursor-pointer card compact bg-base-100"
          key={index}
          href={article.link}
          onClick={(e) => {
            e.preventDefault();
